...
#define ENDEBUG 1
#define DBGPR(fmt,...) do{if(ENDEBUG) printk(fmt,__VA_ARGS__);}while(0)
#define DBGPRS(...)  do{if(ENDEBUG) printk(__VA_ARGS__);}while(0)
#define PR(...) do{if(ENDEBUG) printk("---%s---%d---\n",__FUNCTION__,__LINE__);}while(0)
...
#ifdef CONFIG_JZ4755_D8
static struct mmc_host *jz_mmc;
#endif


/*--------------------------------------------------*/
/*#ifdef (CONFIG_MSC0_JZ4750 && CONFIG_MSC1_JZ4750)
...
#endif
#ifdef SKMD
    printk("Using scan media and return directly.");
    return 0;
#endif

#ifdef CONFIG_JZ4755_D8
	empty = (__msc1_card_detected(slot) == 0) ? 0 : 1;
#else//#ifdef CONFIG_JZ4755_D8
#ifdef CONFIG_MSC1_JZ4750
	empty = (__msc1_card_detected(slot) == 0) ? 1 : 0;
#else
	empty = (__msc0_card_detected(slot) == 0) ? 1 : 0;
#endif
#endif//#ifdef CONFIG_JZ4755_D8

	if (empty) {
		/* wait for card insertion */
#ifdef CONFIG_JZ4755_D8
		__gpio_as_irq_fall_edge(GPIO_MMC_CARD_DETECT_PIN); // forest for iriver cetus board 0617
#else //#ifdef CONFIG_JZ4755_D8
...
#ifdef CONFIG_JZ4755_D8
		__gpio_as_irq_rise_edge(GPIO_MMC_CARD_DETECT_PIN);
#else
#ifdef CONFIG_SOC_JZ4750
#ifdef CONFIG_MSC1_JZ4750
		__gpio_as_irq_fall_edge(MSC_HOTPLUG_PIN);
...
#ifdef CONFIG_JZ4755_D8
	if (jz_mmc) {
		host = mmc_priv(jz_mmc);
		jz_mmc_detect_irq(0, (void *)host);
	}
#endif
	return;
}
...
*/
	...
	if (!jz_mmc_slot_is_empty(0)) {
        printk("Default scanmedia and schedule mmc_rescan.\n");	
        mmc_slot_enable = 1;
